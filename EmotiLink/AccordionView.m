...
- (void)addHeader:(UIButton *)aHeader withView:(id)aView imgView:(UIImageView *)img{
    indexes = [[NSMutableArray alloc]init];
    if ((aHeader != nil) && (aView != nil)) {
        [headers addObject:aHeader];
        [views addObject:aView];
        [lineview addObject:img];
    ...
    if (allowsMultipleSelection) {
        ...
        if ([selectionIndexes containsIndex:[sender tag]]) {
            if (self.allowsEmptySelection || [selectionIndexes count] > 1) {
                ...
                // _imageChange.image=[UIImage imageWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"ndrop_arrow" ofType:@"png"]];
            }
        } else {
            [indexes addObject:[NSNumber numberWithInteger:[sender tag]]];
            if (indexes.count>2) {
                NSInteger newRow = [[indexes objectAtIndex:((indexes.count)-2)] integerValue];
                [mis removeIndex:newRow];
                // _imageChange.image=[UIImage imageWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"ndrop_arrow" ofType:@"png"]];
            }
            
            [mis addIndex:[sender tag]];
            //  _imageChange.image=[UIImage imageWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"ndropdwn_arrow----" ofType:@"png"]];
