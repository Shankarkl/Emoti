//
//  TopRankingListViewController.m
//  EmotiLink
//
...
#import "TopRankingListTableViewCell.h"
...
@interface TopRankingListViewController ()

@end

@implementation TopRankingListViewController
...
- (void)viewDidLoad {
    ...
    [_providerTableViewBackground addSubview:bgImageView];
    [_providerTableViewBackground sendSubviewToBack:bgImageView];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    TopRankingListTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"ProviderCell" forIndexPath:indexPath];
    ...
    if(![appdelegate accessToken] || [appdelegate.accessToken isKindOfClass:[NSNull class]])
    {
        ...
        cell.availableBtnView.hidden = YES;
        cell.availableBtn.hidden = YES;
        
    }else{
        cell.favoriteProviderIcon.hidden = NO;
        cell.availableBtnView.hidden = NO;
    ...
    if([favorite isEqualToNumber:[NSNumber numberWithInt:0]])
    {
        ...
        [cell.favoriteProviderIcon setImage:[UIImage imageWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"favoriteUnselected" ofType:@"png"]] forState:UIControlStateNormal];
    ...
    else
    {
        ...
        ToggleButton *obj=[_values objectAtIndex:indexPath.row];
        obj.selected=YES;
        [cell.favoriteProviderIcon setImage:[UIImage imageWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"favoriteSelected" ofType:@"png"]] forState:UIControlStateSelected];
...
-(IBAction)addToFavorites:(id)sender{
    ...
    TopRankingListTableViewCell *cell=(TopRankingListTableViewCell *)[self.providerTableView cellForRowAtIndexPath:indexPath];
...
-(void)viewWillAppear:(BOOL)animated{
    self.navigationItem.title = @"PROVIDERS RANKING LIST";
    id<GAITracker> tracker = [[GAI sharedInstance] defaultTracker];
    [tracker set:kGAIScreenName value:@"ProviderTopRankingList"];
    [tracker send:[[GAIDictionaryBuilder createScreenView] build]];
    
    if(_providerDataArray || _providerDataArray.count==0)
    {
